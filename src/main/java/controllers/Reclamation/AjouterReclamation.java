...
import services.ServicesReclamation.SendWhatsappMsg;
            ...
            /*if(id!=0){*/
                if ( descriptionService.isEmpty() || reason_combo.getValue()== null|| urgency_combo.getValue()== null){
                    ...
                    if ( filter.hasBadWord(description_id.getText()))
                    {
                        ...
                        Personne personne = gs.getIdUtilisateurByEmail(SessionUser.getUser().getAdresseEmail());
                        Reclamation service = new Reclamation(personne.getId(), titre_r,descriptionService, "pending", urgence ,currentDate);
                        rs.ajouter_Reclamation(service);
                        SendWhatsappMsg.sendWhatsappMsg(personne);
